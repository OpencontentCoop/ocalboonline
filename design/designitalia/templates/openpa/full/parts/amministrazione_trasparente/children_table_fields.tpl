...
{else}
    ...
    $(document).ready(function () {ldelim}
      ...
      var fieldsDatatable = $('#container-{$node.node_id}-{$index}').opendataDataTable({ldelim}
    ...
    <div id="container-{$node.node_id}-{$index}"></div>
    ...
    <div class="Callout Callout--could u-text-r-xs u-margin-top-m u-margin-bottom-m">
		...
		Ottieni questi contenuti in <a href="{concat("exportas/custom/csv_trasparenza/", $fields.query|urlencode, "/",$node.node_id)|ezurl(no)}">formato <abbr title="Comma-separated values">CSV</abbr></a> o in <a href="{concat("/api/opendata/v2/content/search/",$fields.query|urlencode)}">formato <abbr title="JavaScript Object Notation">JSON</abbr></a>            
	</div>
