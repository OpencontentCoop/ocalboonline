...
class CheckTrasparenzaTool extends BaseTrasparenzaTool
{
    ...
    )
    {
        try{
            ...
            $this->checkPriority($mainNode, $priority);
    ...
    {
        $results = $this->search->search("titolo = '" . addcslashes($remoteName, "'") . "' and classes [pagina_trasparenza] and subtree [1]");
        if (count($results->searchHits) > 0) {
            ...
            if (!empty($options)) {
                ...
                if ($choice > 0) {
                    ...
                    eZCLI::instance()->warning("Cambia remoteId da $remoteId a " . $remoteRemoteId . ' al nodo ' . $object->attribute('main_node_id'));
                }
